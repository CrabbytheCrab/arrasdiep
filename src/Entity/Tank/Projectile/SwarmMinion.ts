 ...
 */
export default class SwarmMinion extends Drone implements BarrelBase {
    /** Size of the focus the minions orbit. */
    public static FOCUS_RADIUS = 425 ** 2;
    ...
    protected tickMixin(tick: number) {
        ...
        if (usingAI && this.ai.state === AIState.idle) {
            ...
            if (dist < SwarmMinion.FOCUS_RADIUS / 4) { // Half
                this.movementAngle = this.positionData.values.angle + Math.PI;
            } else if (dist < SwarmMinion.FOCUS_RADIUS) {
