        ...
        switch (header) {
            case ServerBound.Input: {
                ...
                if (flags & InputFlags.levelup) {
                    ...
                    if ((this.accessLevel === config.AccessLevel.FullAccess) || (camera.cameraData.values.level < config.maxPlayerLevel && ((this.game.arena.arenaData.values.flags & ArenaFlags.canUseCheats) || (this.accessLevel === config.AccessLevel.BetaAccess) || (this.accessLevel === config.AccessLevel.PublicAccess)))) {
                        this.setHasCheated(true);
